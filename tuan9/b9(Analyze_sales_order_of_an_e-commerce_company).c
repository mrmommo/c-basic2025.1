...
long long get_shop_sum(char* shop) {
    ...
    ShopNode* node = shop_hash[h];
    while(node) {
        if(strcmp(node->shop, shop) == 0) return node->sum;
        node = node->next;
    }
    return 0;
}
...
long long get_custshop_sum(char* customer, char* shop) {
    ...
    CustShopNode* node = custshop_hash[h];
    while(node) {
        if(strcmp(node->key, key) == 0) return node->sum;
        node = node->next;
    }
    return 0;
}
...
int main() {
    ...
    while(fgets(line, sizeof(line), stdin)) {
        ...
        int price;
        sscanf(line, "%s %s %d %s %s", customer, product, &price, shop, time_str);
    ...
    // prepare time
    for(int i = 0; i < num_orders; i++) order_indices[i] = i;
    ...
    while(fgets(line, sizeof(line), stdin)) {
        ...
        if(strcmp(query, "?total_number_orders") == 0) {
            ...
            while(low < high) {
                int mid = (low + high) / 2;
                if(orders[order_indices[mid]].time_sec >= from_sec) high = mid;
                else low = mid + 1;
            }
            int start = low;
            ...
            while(low < high) {
                int mid = (low + high) / 2;
                if(orders[order_indices[mid]].time_sec > to_sec) high = mid;
                else low = mid + 1;
            }
            int end = low;
